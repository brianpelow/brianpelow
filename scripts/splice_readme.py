...
- Output is asserted pure-ASCII before writing, matching the generator that
  produces the block.
...
REGIONS = {"CATALOG": "catalog_block.md"}
TIMEOUT = 30
...
def splice(text: str, region: str, block: str) -> str:
    ...
    if start not in text or end not in text:
        raise SystemExit(f"FAIL: markers for {region} not found in README.md")
...
def main() -> int:
    ...
    print(f"[splice] wrote README.md ({len(text)} chars)")
    return 0
